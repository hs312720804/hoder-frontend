<template>
    <div>
      ...
      <!-- pagination -->
        ...
        <!--新增界面-->
        <el-dialog title="新增" :visible.sync="addFormVisible" v-if="addFormVisible" :close-on-click-modal="false" size="large" >
            <el-row :gutter="40">
                <el-col :span="12">
        ...
        <!--编辑界面-->
        <el-dialog title="编辑" :visible.sync="editFormVisible" v-if="editFormVisible" v-model="editFormVisible" :close-on-click-modal="false" size="large">
            <el-row :gutter="40">
                <el-col :span="12">
                    <el-form :model="roleForm" :rules="roleFormRules" ref="roleForm" label-width="100px">
                        <el-form-item label="角色" prop="name">
...
export default {
    data(){
        ...
        created () {
            this.loadData();
            this.$service.menuTree().then((data)=>{
              this.data2=data.data;
              this.initTree = this.searchInit(this.data2);
            })
        },
        methods: {
            ...
            handleChangetStatus: function (index, row) {
                ...
                this.$confirm('确定修改该条记录的状态?', '提示', {
                    ...
                    type: 'warning'
                     ...
                     this.$service.changeRoleStatus({id:id,useable:useable}).then((data)=>{
            ...
            handleBatchDel: function () {
                ...
                if (ids.length > 0) {
                    this.$confirm('确定要删除这批记录?', '提示', {
                      ...
                      this.$service.batchRoles(ids).then((data)=>{
                        this.callback(data,"删除成功");
                      })
                    }).catch(() => {
                        this.$message({
