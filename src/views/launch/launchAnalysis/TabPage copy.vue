  ...
  <div class="my-collect">
    ...
    <div class="container">
      <div v-for="index in 10" :key="index">
        ...
        <div
          class="box"
          :style="{left: Math.sin(ahd*index)*radius+dotLeft+'px', top: Math.cos(ahd*index)*radius+dotTop+'px'}"
        >
        </div>
        <!-- {{ Math.sin((ahd*index))*radius+dotLeft }}
        {{ Math.cos((ahd*index))*radius+dotTop }} -->
      </div>
    </div>
...
export default {
  ...
  created () {
    ...
    // // 中心点横坐标
    // this.dotLeft = (500 - 30) / 2
    // // 中心点纵坐标
    // this.dotTop = (500 - 30) / 2
  ...
  methods: {
    returnStyle (index) {
      ...
      const dotLeft = this.dotLeft
      const dotTop = this.dotTop
      const left = Math.sin(ahd * index) * radius + dotLeft + 'px'
      const top = Math.cos(ahd * index) * radius + dotTop + 'px'
