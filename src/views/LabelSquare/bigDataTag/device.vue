<template>
  <div class="label-zone">
    ...
    <div v-if="filter.tagName === undefined || filter.tagName === ''">
      ...
      <div v-for="(item, index) in treeData" :key="item.parentId" :id="'tab-content-' + index" class="tab-content">

        <div class="title">{{ item.parentName }}<span v-if="item.newOrUpdateCount" class="small-red">·</span></div>
        <el-tabs v-model="activeTab" @tab-click="handleTabClick(index)">
          <el-tab-pane v-for="childItem in item.children" :key="childItem.groupId" :name="childItem.groupId">
            <span slot="label">{{ childItem.groupName }}<span v-if="childItem.newOrUpdateCount"
                class="small-red">·</span></span>
